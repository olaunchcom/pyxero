...
from .exceptions import ApiException, ApiSSLException, HTTPStatusException
...
class RESTResponse(io.IOBase):
    ...
    @property
    def text(self):
        """Get response data as text string"""
...
class RESTClientObject(object):
    ...
    def request(
        ...
        if _request_timeout:
            timeout_classes = (int,)
            if isinstance(_request_timeout, timeout_classes):
        ...
        try:
            ...
            if method in ["POST", "PUT", "PATCH", "OPTIONS", "DELETE"]:
                ...
                if re.search("json", headers["Content-Type"], re.IGNORECASE):
                    ...
                    fields = {post_params[2][1]: (post_params[1][1], body, None)}
                    r = self.pool_manager.request(
                        method,
                        url,
                        fields=fields,
            ...
            raise ApiSSLException(status=0, reason=msg)

        if not 200 <= r.status <= 299:
            raise HTTPStatusException(http_resp=RESTResponse(r))
